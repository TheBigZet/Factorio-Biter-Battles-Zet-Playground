...
local function clear_corpses(cmd)
    ...
    if not trusted[player.name] and not player.admin and param > untrusted_max_radius then
        player.print('[INFO] Replaced radius with max allowable value for untrusted players: ' .. untrusted_max_radius, { color = Color_warning })
    ...
    if param > 500 then
        player.print('[INFO] Replaced radius with max allowable value:  ' .. trusted_max_radius, { color = Color_warning })
